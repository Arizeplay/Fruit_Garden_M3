...
public class InAppPurchasesEditor : MetaEditor<BerryStore> {
    ...
    public override void OnGUI() {
        ...
        {
            var path = EditorUtility.SaveFolderPanel("Export IAP", "", "");
            if (path.Length > 0)
            {
                using var writer = new StreamWriter(Path.Combine(path, "IAP.json"));
                var json = JsonConvert.SerializeObject(metaTarget.iaps);
        ...
        using (new GUIHelper.Horizontal()) {
			GUILayout.Space(20);
			GUILayout.Label("Android ID", Styles.centeredMiniLabel, GUILayout.Width(100));
            GUILayout.Space(20);
            GUILayout.Label("IOS ID", Styles.centeredMiniLabel, GUILayout.Width(100));
            GUILayout.Space(20);
        ...
        foreach (BerryStore.IAP iap in metaTarget.iaps) {
            using (new GUIHelper.Horizontal()) {
                ...
                iap.androidId = EditorGUILayout.TextField(iap.androidId, GUILayout.Width(100));

                GUILayout.Space(20);

				iap.iosId = EditorGUILayout.TextField(iap.iosId, GUILayout.Width(100));
...
public class StoreEditor : MetaEditor<BerryStore> {
    ...
    void DrawItem(BerryStore.Item item) {
        using (new GUIHelper.Horizontal(GUILayout.ExpandHeight(true))) {
            ...
            if (item.icons.Count == 0)
            {
                item.icons = new List<BerryStore.Item.Icon>();
                item.icons.Add(new BerryStore.Item.Icon());
            }
            
            if (item.icons[0] != null) {
                Texture2D preview = AssetPreview.GetAssetPreview(item.icons[0].icon);
                if (preview != null) {
                    ...
                    case 0: {
                            ...
                            switch (item.purchaseType) {
                                case BerryStore.Item.PurchaseType.IAP:
                                    {
                                        item.iap = IAP_ids.Get(EditorGUILayout.Popup("IAP ID", IAP_ids.IndexOf(item.iap), IAP_ids.ToArray(), GUILayout.ExpandWidth(true))); 

                                        break;
                                    }
                                case BerryStore.Item.PurchaseType.CoinCurrency:
                                    {
                                        item.cost = Mathf.Max(1, EditorGUILayout.IntField("Cost (Coins)", item.cost, GUILayout.ExpandWidth(true)));
                                        break;
                                    }
                            ...
                            item.showInfoButton = EditorGUILayout.Toggle("Show Info Button", item.showInfoButton, GUILayout.ExpandWidth(true));
                            item.showTape = EditorGUILayout.Toggle("Show Tape", item.showTape, GUILayout.ExpandWidth(true));
                            if (item.showTape)
                            {
                                item.tapeDescription = EditorGUILayout.TextField("Tape Description", item.tapeDescription, GUILayout.ExpandWidth(true));
                            }

                            for (int iconIndex = 0; iconIndex < item.icons.Count; iconIndex++) {
                                GUILayout.BeginHorizontal();
                                item.icons[iconIndex].icon = (Sprite) EditorGUILayout.ObjectField($"Icon {iconIndex}", item.icons[iconIndex].icon, typeof(Sprite), GUILayout.ExpandWidth(true));
                                GUILayout.BeginVertical();
                                item.icons[iconIndex].localized = EditorGUILayout.Toggle("Localized", item.icons[iconIndex].localized, GUILayout.ExpandWidth(true));
                                if(!item.icons[iconIndex].localized)
                                    item.icons[iconIndex].description = EditorGUILayout.TextField("Description", item.icons[iconIndex].description, GUILayout.ExpandWidth(true));
                                GUILayout.EndVertical();
                                GUILayout.EndHorizontal();
                            }

                            GUILayout.BeginHorizontal();
                            if (item.icons.Count > BerryStore.Item.MinIcons) {
                                if (GUILayout.Button("-", GUILayout.Width(30))) {
                                    item.icons.Remove(item.icons.Last());
                                }
                            }
                        
                            if (item.icons.Count < BerryStore.Item.MaxIcons) {
                                if (GUILayout.Button("+", GUILayout.Width(30))) {
                                    item.icons.Add(new BerryStore.Item.Icon());
                                }
                            }
                    ...
                    case 2: {
                            ...
                            for (int i = 0; i < item.pack.Count; i++) {
                                using (new GUIHelper.Horizontal()) {
                                    ...
                                    item.pack[i].type = (BerryStore.ItemPack.Type) EditorGUILayout.EnumPopup(item.pack[i].type, EditorStyles.miniButtonRight, GUILayout.Width(100));
                                    switch (item.pack[i].type)
                                    {
                                        case BerryStore.ItemPack.Type.Items:
                                            ...
                                            break;
                                        case BerryStore.ItemPack.Type.Moves:
                                            item.pack[i].movesCount = Mathf.Max(1, EditorGUILayout.IntField(item.pack[i].movesCount, GUILayout.MaxWidth(100)));
                                            break;
                                        default:
    ...
    void DrawGroup(BerryStore.Group group) {
        using (new GUIHelper.Vertical(GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true))) {
            ...
            group.IsItemGroup = EditorGUILayout.Toggle("Is Item Group", group.IsItemGroup, GUILayout.ExpandWidth(true));
            if (group.IsItemGroup)
            {
                group.ItemGroup = (ItemID)EditorGUILayout.EnumPopup("Item Group ID", group.ItemGroup, GUILayout.ExpandWidth(true));
            }
    ...
    class StoreTree : GUIHelper.HierarchyList<BerryStore.Item, BerryStore.Group> {
        ...
        public override void DrawFolder(Rect rect, FolderInfo info) {
            if (!groupIcon) groupIcon = EditorIcons.GetBuildInIcon("StoreGroupIcon");
        ...
        public override void DrawItem(Rect rect, ItemInfo info) {
            if (!adsIcon) adsIcon = EditorIcons.GetBuildInIcon("StoreAdsIcon");
            if (!iapIcon) iapIcon = EditorIcons.GetBuildInIcon("StoreIAPIcon");
            if (!itemIcon) itemIcon = EditorIcons.GetBuildInIcon("StoreItemIcon");
            if (!lockIcon) lockIcon = EditorIcons.GetBuildInIcon("UnlockedIcon");
            ...
            switch (info.content.purchaseType) {
                ...
                case BerryStore.Item.PurchaseType.CoinCurrency: GUI.DrawTexture(_rect, itemIcon); break;
            }
            _rect.x += 16;
...
[CustomEditor(typeof(MaskLibrary))]
public class PageMaskEditor : Editor {

    MaskLibrary main;

    void OnEnable() {
        main = (MaskLibrary) target;
    }

    public override void OnInspectorGUI() {
        Undo.RecordObject(main, "PageMask changes");

        main.type = (MaskLibraryComparison) EditorGUILayout.EnumPopup("Type", main.type);

        switch (main.type)
        {
            case MaskLibraryComparison.Page:
                DrawPageValue(main.PageA);
                main.simpleMustBe = (SimpleComparisonOperator) EditorGUILayout.EnumPopup("Mast be", main.simpleMustBe);
                DrawPageValue(main.PageB);
                break;
            case MaskLibraryComparison.PlayingMode:
                DrawPlayingModeValue(main.PlayModeA);
                main.simpleMustBe = (SimpleComparisonOperator) EditorGUILayout.EnumPopup("Mast be", main.simpleMustBe);
                DrawPlayingModeValue(main.PlayModeB);
                break;
            case MaskLibraryComparison.Field:
                main.fieldMustBe = (FieldComparisonSource) EditorGUILayout.EnumPopup("Mast be", main.fieldMustBe);
                break;
            case MaskLibraryComparison.Difficulty:
                EditorGUILayout.LabelField("Current Level Difficulty");
                main.operators = (Operators) EditorGUILayout.EnumPopup("Mast be", main.operators);
                main.difficulty = (LevelDesign.Difficulty) EditorGUILayout.EnumPopup("Difficulty", main.difficulty);
                break;
        }
    ...
    private void DrawPageValue(MaskLibrary.PageValue a) {
        using (new GUIHelper.Vertical(Styles.area)) {
			a.source = (PageComparisonSource) EditorGUILayout.EnumPopup("Source", a.source);

			switch (a.source) {
				case PageComparisonSource.CurrentPage: break;
				case PageComparisonSource.Page: a.page = EditorGUILayout.TextField("Page", a.page); break;
            }
        }
    }
    
    private void DrawPlayingModeValue(MaskLibrary.PlayModeValue a) {
        using (new GUIHelper.Vertical(Styles.area)) {
            a.source = (PlayingModeComparisonSource) EditorGUILayout.EnumPopup("Source", a.source);

            switch (a.source) {
                case PlayingModeComparisonSource.CurrentPlayMode: break;
                case PlayingModeComparisonSource.PlayMode: a.playMode = (PlayingMode) EditorGUILayout.EnumPopup("PlayingMode", a.playMode); break;
            }
        }
    }
}
