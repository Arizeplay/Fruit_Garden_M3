...
public class GeneratorEditor : SlotEditorExtension {
    ...
    public override void OnSlotEditorGUI(LevelDesign design, Dictionary<int2, SlotContent> content)
    {
        if (SlotGenerator.generatedChips.Count > 0) {
            _move = EditorGUILayout.IntSlider("Move",_move, 0, design.movesCount);
            ...
            foreach (var pair in SlotGenerator.probabilityPrefabs) {
                ...
                EUtils.DrawMixedProperty(content.Keys,
                    mask: coord => true,
                    getValue: coord => content[coord][variable].Curve,
                    setValue: (coord, value) => content[coord][variable].Curve = value,
                    drawSingle: (position, value) =>
                    {
                        content[position][variable].Curve ??= AnimationCurve.Constant(0, 1, 0);

                        return EditorGUILayout.CurveField(Math.Round(content[position][variable].Curve.Evaluate((float)_move / design.movesCount), 2) + " " + title, value);

                    },
                    drawMixed: setDefault =>
                    {
                        var value = EditorGUILayout.CurveField(title, AnimationCurve.Constant(0, 1, 0));
                        if (value.Equals(new AnimationCurve())) return false;
                        setDefault(value);
                        return true;
                    });
            }
