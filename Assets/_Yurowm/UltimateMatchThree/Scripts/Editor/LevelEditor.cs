...
public class LevelEditor : MetaEditor<LevelAssistant> {
    ...
    private static List<IBooster> booster;
    private static string[] boosterNames;
    
    public LevelDesignFile designFile;
    public LevelDesign design {
        get {
            return designFile == null ? null : designFile.FullDesign;
        }
    }
    ...
    private SecondaryLevelDesign _copy;
    ...
    public override bool Initialize() {
        ...
        #region Icons
        slotIcon = EditorIcons.GetBuildInIcon("SlotIcon");
        chipIcon = EditorIcons.GetBuildInIcon("ChipIcon");
        blockIcon = EditorIcons.GetBuildInIcon("BlockIcon");
        blockColorIcon = EditorIcons.GetBuildInIcon("BlockColorIcon");
        
        listFolderIcon = EditorIcons.GetBuildInIcon("Folder");
        listLevelIcon = EditorIcons.GetBuildInIcon("LevelIcon");

        listChipIcon = EditorIcons.GetBuildInIcon("ChipIconMini");
        listBlockIcon = EditorIcons.GetBuildInIcon("BlockIconMini");
        listModifierIcon = EditorIcons.GetBuildInIcon("ModifierIconMini");
        listExtensionIcon = EditorIcons.GetBuildInIcon("ExtensionIcon");
        #endregion
        ...
        booster = Content.GetPrefabList<IBooster>();
        boosterNames = booster.Select(x => (x as IBooster).name).ToArray();
    ...
    void DrawLevelParameters()
    {
        if (design != null) {
            ...
            using (new GUIHelper.Horizontal()) {
                if (GUILayout.Button("Copy",  EditorStyles.miniButtonRight, GUILayout.Width(50)))
                {
                    _copy = design.GetCurrentSecondaryLevelDesign();
                }
                        
                using (new GUIHelper.Lock(_copy == null))
                    if (GUILayout.Button("Paste",  EditorStyles.miniButtonRight, GUILayout.Width(50)) && 
                            EditorUtility.DisplayDialog("Paste", "Are you sure want to paste the level", "Paste", "Cancel"))
                    {
                        design.SetSecondaryLevelDesign(design.difficulty, _copy.Clone());
                        UpdateSelectedLevel();
                    }
            ...
            #region Copy
            GUILayout.Label("Copy Design", Styles.centeredMiniLabel, GUILayout.ExpandWidth(true));

            #endregion
            
            #region Difficulties
            using (new GUIHelper.BackgroundColor(propertiesBackgroundColor)) {
                using (new GUIHelper.Horizontal(Styles.area, GUILayout.ExpandWidth(true))) {
                    foreach (var difficulty in Enum.GetValues(typeof(LevelDesign.Difficulty)))
                    {
                        using (new GUIHelper.BackgroundColor(Color.Lerp(Color.gray, Color.white, design.difficulty == (LevelDesign.Difficulty)difficulty ? 1 : .33f)))
                            if (GUILayout.Button(difficulty.ToString()))
                            {
                                design.difficulty = (LevelDesign.Difficulty)difficulty;
        ...
        using (new GUIHelper.Vertical(Styles.area)) {
            ...
            if (ruleSelector.Count > 0) {
                ...
                GUI.contentColor = new Color(0.9f, 0.5f , 0.4f);
                DrawStarPanel();
                GUI.contentColor = Color.white;
    ...
    public void SelectLevel(LevelDesignFile designFile) {
        if (designFile == null || designFile.FullDesign == null)
            return;
    ...
    internal void RunLevel() {
        ...
        PlayerPrefs.SetInt("TestLevelDifficulty", (int)designFile.HalfDesign.difficulty);
    }
    ...
    public class LevelDesignFile {
        ...
        private LevelDesign design = null;
        ...
        public bool IsFullDesign() {
            return design.NormalLevelDesign != null && design.NormalLevelDesign.slots != null && design.NormalLevelDesign.slots.Count > 0;
        }
        
        public LevelDesign FullDesign {
            get {
                if (!IsLoaded() || !IsFullDesign()) Load(true, false, true);
                return design;
            }
        }
        
        public LevelDesign HalfDesign {
            get {
                if (!IsLoaded()) Load(true, false, false);
                return design;
            }
        }
        public string Path {
        ...
        public void Load(bool loadDesign, bool loadAttributes, bool loadFull = false) {
            ...
            if (loadDesign) {
                ...
                try
                {
                    design = loadFull ? LevelDesign.DeserializeFull(xml) : LevelDesign.DeserializeHalf(xml);
                } catch (Exception e) {
    ...
    public class LevelList : GUIHelper.HierarchyList<LevelDesignFile> {
        ...
        void Import() {
            ...
            foreach (var xLevel in xml.Root.Elements()) {
                LevelDesign level = LevelDesign.DeserializeHalf(xLevel);
                levels.Add(level);
            }
        ...
        void Export(IInfo[] selection) {
            ...
            foreach (var info in selection) {
                ...
                xml.Add(info.asItemKind.content.FullDesign.Serialize("level"));
                save = true;
            }
            if (!save) {
        ...
        void Dublicate(List<ItemInfo> levels, FolderInfo folder) {
            List<LevelDesign> clones = levels.Select(x => x.content.FullDesign.Clone()).ToList();
    ...
    class GoalList : GUIHelper.NonHierarchyList<LevelGoalInfo> {
        ...
        public override void DrawItem(Rect rect, ItemInfo info) {
            if (goalIcon == null) goalIcon = EditorIcons.GetBuildInIcon("GoalIcon");
    ...
    class ExtensionList : GUIHelper.NonHierarchyList<LevelExtensionInfo> {
        ...
        public override void DrawItem(Rect rect, ItemInfo info) {
            if (extensionIcon == null) extensionIcon = EditorIcons.GetBuildInIcon("ExtensionIcon");
