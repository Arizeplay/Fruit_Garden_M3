...
public abstract class IBlock : ISlotContent {
    /// <summary>
    /// Short word, which is used for indicating the blocker in the level editor (max. 2 letters is recommended)
    /// </summary>
    /// <returns></returns>
...
public abstract class ILevelExtension : ILiveContent {
    ...
    public static readonly List<ILevelExtension> currentExtensions = new List<ILevelExtension>();

    public virtual void Setup(LevelExtensionInfo info)
    {
        currentExtensions.Add(this);
    }
    
    public override void OnKill()
    {
        if (currentExtensions.Contains(this))
            currentExtensions.Remove(this);
    }
