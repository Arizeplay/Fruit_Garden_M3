...
public class CrossExplosionMix : IChipMix {
    ...
    public override IEnumerator Destroying() {
        ...
        while (sides.Count > 0) {
            foreach (Side side in Utils.allSides) {
                if (sides.Contains(side)) {
                    ...
                    score += CrossBomb.Hit(coord, 0, Side.Null, hitted, context);
                    for (int o = 1; o <= offset; o++) {
                        score += CrossBomb.Hit(coord, o, side.RotateSide(-2), hitted, context);
                        score += CrossBomb.Hit(coord, o, side.RotateSide(2), hitted, context);
                    }
                }
            }
        ...
        ScoreEffect.ShowScore(transform.position, score, color, 2f);
    }
