...
public class SessionAssistant : MonoBehaviourAssistant<SessionAssistant>
{
    ...
    public static Action<LevelDesign> OnRunLevelDesign;
    ...
    void OnApplicationPause(bool pauseStatus)
    {
        if (SessionInfo.current != null && SessionInfo.current.isPlaying)
            UIAssistant.main.ShowPage("Settings");
    }
    ...
    public void SkipCompleteBonus()
    {
        if (CPanel.uiAnimation != 0)
            return;
        
        SessionInfo.current.rule.KillBonus();
    }
    ...
    public void StartSession(LevelDesign design, LevelDesign.Difficulty difficulty)
    {
        LevelDesign.selected = design;
        StartSession(new SessionInfo(design, difficulty));
    }

    public void StartSession(SessionInfo info = null)
    {
        StopAllCoroutines();
        
        SessionInfo.current = info ?? new SessionInfo(LevelDesign.selected, SessionDirector.GetDifficulty(LevelDesign.selected));
        
        OnRunLevelDesign?.Invoke(SessionInfo.current.design);
        
        Project.onLevelEnd.AddListener(() => OnLevelEnd?.Invoke());
        
        ///
        {
            AppMetrica app = FindObjectOfType<AppMetrica>();
            string json = $"{{\"Level {SessionInfo.current.design.number}\":\"1\"}}";
            app.SendEvent("Open Level", json);
            app.RegisterEvents();
        }
        ///

        Advertising.instance.RegisterEvents();
...
public class SessionInfo
{
    ...
    public SessionInfo(LevelDesign levelDesign, LevelDesign.Difficulty? difficulty = null) : this()
    {
        SetDesign(levelDesign.Clone());

        _design.difficulty = difficulty ?? LevelDesign.Difficulty.Normal;
    ...
    public void SetMask()
    {
        if (!IsBaked() && design != null)
        {
            ...
            for (int key = 0; key < design.colorCount; key++)
                colorMask.Add(ItemColorUtils.physiscalColors[key], unsorted[key]);
        }
    }
    ...
    public IBooster startBooster = null;
    public IBooster mBooster = null;

    public bool isPlaying = false;
    
    public BoostersUsed boostersUsed = new BoostersUsed();
    
    public class BoostersUsed
    {
        private Dictionary<ItemID, int> _stats;

        public BoostersUsed()
        {
            _stats = new Dictionary<ItemID, int>();
        }
        ...
        public int this[ItemID index]
        {
            ...
            set
            {
                ...
                _stats.Set(index, value);
            }
        }

        public ItemID[] usedItems => _stats.Keys.ToArray();
        ...
        public void Serialize(XElement xml)
        {
            foreach (var key in _stats.Keys)
            {
                var element = new XElement("item",
                    new XAttribute("id", (int) key), 
                    new XAttribute("value", _stats[key]));
        ...
        public void Deserialize(XElement xml)
        {
            ...
            foreach (var element in xml.Descendants("item"))
            {            
                var itemID = (ItemID) int.Parse(element.Attribute("id").Value);
                var value = int.Parse(element.Attribute("value").Value);

                _stats.Add(itemID, value);
            }
        }
    }

    public MoveTimer moveTimer = new MoveTimer();
    public class MoveTimer
    {
        public List<int> list;
        public float oldMoveTime;
        ...
        public void Move()
        {
            ...
            var delay = currentMoveTime - oldMoveTime;
        ...
        public void Serialize(XElement xml)
        {
            ...
            {
                var time = list[index];
                var element = new XElement("move_timer",
                    new XAttribute("move", index),
        ...
        public void Deserialize(XElement xml)
        {
            ...
            foreach (var element in xml.Descendants("move_timer"))
            {            
                var move = int.Parse(element.Attribute("move").Value);
                var time = int.Parse(element.Attribute("time").Value);
    ...
    public string ToXml()
    {
        ...
        XElement boostersUsed = new XElement("boostersUsed");
        XElement moveTimer = new XElement("moveTimer");
        ...
        root.Add(reactions);
        root.Add(extensions);
        root.Add(boostersUsed);
        root.Add(moveTimer);
        
        this.rule.Serialize(rule);
        this.boostersUsed.Serialize(boostersUsed);
        this.moveTimer.Serialize(moveTimer);
    ...
    public static SessionInfo FromXml(string raw)
    {
        ...
        try
        {
            ...
            var rawIndex = LevelAssistant.main.GetDesign(level_number).rawDesignIndex;
            var levelRaw = LevelAssistant.levelsRaw[rawIndex];
            LevelDesign design = LevelDesign.DeserializeFull(XElement.Parse(levelRaw));
            ...
            XElement boostersUsed = root.Element("boostersUsed");
            XElement moveTimer = root.Element("moveTimer");
            ...
            session.boostersUsed.Deserialize(boostersUsed);
            session.moveTimer.Deserialize(moveTimer);
    ...
    internal void Save(bool force = false)
    {
        ...
        {
            PlayerPrefs.SetString(sessionSaveKey, ToXml());
            PlayerPrefs.Save();
        }
    }
