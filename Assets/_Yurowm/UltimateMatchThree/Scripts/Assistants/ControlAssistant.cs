...
public class ControlAssistant : MonoBehaviour {
    ...
    public IEnumerator ControlRoutine(LevelRule rule) {
        while (SessionInfo.current != null) {
            ...
            while (IsControl()) {
                switch (currentMode) {
                    case ControlMode.Regular:
                        {
                            onControl?.Invoke(isBegan(), isPress(), IsOverUI(), isPress() ? getPoint() : Vector2.zero);
                            rule.Control(isBegan(), isPress(), IsOverUI(), isPress() ? getPoint() : Vector2.zero); break;
                        }
    ...
    public delegate void OnControl(bool isBegan, bool isPress, bool isOverUI, Vector2 point);
    public OnControl onControl;
