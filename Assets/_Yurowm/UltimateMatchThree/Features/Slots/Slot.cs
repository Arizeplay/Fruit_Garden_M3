...
public class Slot : ILiveContent, IAnimated {
    ...
    public List<Slot> GetClosestSlots()
    {
        var slots = new List<Slot>();
        
        if (nearSlot.ContainsKey(Side.Top))
        {
            slots.Add(nearSlot[Side.Top]);
        }
        
        if (nearSlot.ContainsKey(Side.Bottom))
        {
            slots.Add(nearSlot[Side.Bottom]);
        }
        
        if (nearSlot.ContainsKey(Side.Left))
        {
            slots.Add(nearSlot[Side.Left]);
        }
        
        if (nearSlot.ContainsKey(Side.Right))
        {
            slots.Add(nearSlot[Side.Right]);
        }
...
public class RealisticChipPhysic : IChipPhysic {
    ...
    public override Slot GetFallingSlot(Slot currentSlot) {
        ...
        target = currentSlot[targetSide];
        if (target && (currentSlot[lockerSide].block || (!topLocked && !straightSlots.Contains(target))) && IsAvailableForFalling(target)) return target;
