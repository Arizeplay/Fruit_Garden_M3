...
public class ActionTutorial : ActionInfo {
    ...
    static TutorialPopup popup = null;

    public override IEnumerator Logic() {
        string text = localized ? LocalizationAssistant.main[string.Format(localizationKeyPath, ActionSettings.KeyPath, key)] : this.text;
        popup.Say(text, iconName);

        if (!popup.gameObject.activeSelf)
            yield return dialogue.StartCoroutine(popup.Show());
        
        while (!popup.clicked)
            yield return 0;

        yield return dialogue.StartCoroutine(popup.Hide());
    }

    public override void OnDialogueStart() {
        if (!popup) {
            popup = Content.Emit<TutorialPopup>();
            popup.transform.SetParent(dialogue.canvas.transform);
            RectTransform rect = popup.transform.rect();
    ...
    public override void Deserialize(XElement xml) {
        XAttribute attribute = xml.Attribute("iconName");
        if (attribute != null) iconName = attribute.Value;
    ...
    public override void Serialize(XElement xml) {
        xml.Add(new XAttribute("iconName", iconName));
    ...
    public override IEnumerator<KeyValuePair<string, string>> GetState() {
        yield return new KeyValuePair<string, string>("Last Tutorial", (localized ? "@" + key : text));
    }

    public override string GetDetails() {
        return (localized ? "@" + key : text);
    }
}
