...
public class CrossBomb : IChip, IColored, IBomb, INeedToBeSetup {
    ...
    public IEnumerator Destroying() {
        ...
        while (sides.Count > 0) {
            ...
            foreach (Side side in Utils.allSides) {
                if (sides.Contains(side)) {
                    ...
                    score += Hit(coord, 0, Side.Null, hitted, context);
                    for (int o = 1; o <= offset; o++) {
                        score += Hit(coord, o, side.RotateSide(-2), hitted, context);
                        score += Hit(coord, o, side.RotateSide(2), hitted, context);
                    }
                }
            }
        ...
        ScoreEffect.ShowScore(transform.position, score, color, 2.5f);
    ...
    internal static int Hit(int2 coord, int offset, Side offsetSide, List<int2> hitted, HitContext context) {
        int2 _coord = coord;
        int score = 0;
        if (offset > 0)
        ...
        if (!hitted.Contains(_coord)) {
            ...
            if (Slot.allActive.ContainsKey(_coord))
                score += Slot.allActive[_coord].Hit(context);
        }
