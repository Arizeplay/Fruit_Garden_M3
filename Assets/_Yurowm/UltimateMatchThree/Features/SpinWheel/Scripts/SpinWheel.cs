...
public class SpinWheel : MonoBehaviour, ISounded
{
    ...
    // TODO: Сделать событие изменения доступности рекламы и засунуть тогда эту функцию.
    void RewardedRefresh()
    {
        var spinLessMax = CurrentUser.main.rewardedSpin.IsSpinLessMax();
        var isAdTokenMax = CurrentUser.main.dailyToken.IsAdTokenMax();
        var available = CurrentUser.main.rewardedSpin.IsAvailable();
        rewardedSpinButton.gameObject.SetActive(available || !spinLessMax);
        rewardedSpinButton.interactable = available && Advertising.main.CountOfReadyAds(AdType.WheelOfFortune) > 0;
        rewardedSpinTimer.SetActive(!available && !isAdTokenMax && spinLessMax);
    }
    ...
    IEnumerator Spinning()
    {
        ...
        if (pressed)
        {
            ...
            target_angle -= 360 * 3;
            target_angle += Random.Range(-.5f, .5f) * sectorSize + 90;
