...
public class SlotGeneratorExtendedEditor : SlotEditorExtension {
    ...
    public override void OnSlotEditorGUI(LevelDesign design, Dictionary<int2, SlotContent> info) {
        ...
        time = EditorGUILayout.IntSlider("Move",time, 0, design.movesCount);

        int move = time;
        float totalWeight = casesDict.Values.Sum(c => c[SlotGeneratorExtended.weight_curve_parameter].Curve.Evaluate((float) move / design.movesCount));
        foreach (var c in casesDict) {
            using (new GUIHelper.Change(() => c.Key.Text = c.Value.Serialize("case").ToString())) {
                ...
                weight = c.Value[SlotGeneratorExtended.weight_curve_parameter];
                ...
                using (new GUIHelper.Horizontal()) {
                    ...
                    if (GUILayout.Button(prefab.name + " (" + (100f * weight.Curve.Evaluate((float) move / design.movesCount) / totalWeight).ToString("F1") + "%)", EditorStyles.miniButtonRight, GUILayout.ExpandWidth(true))) {
                ...
                if (currentCaseName == c.Key.name) {
                    weight.Curve = EditorGUILayout.CurveField("Weight", weight.Curve);
    ...
    private void AddNewCase(SlotContent current, IChip _chip) {
        ...
        slotContent[SlotGeneratorExtended.weight_curve_parameter].Curve = AnimationCurve.Constant(0,1,0);
